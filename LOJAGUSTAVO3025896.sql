...
-- Nome do aluno:Luiz Gustavo Alvino da Silva
-- Data da prova:02/12/24
-----------------------------------------------------------------------------------
...
CREATE DATABASE LOJAGUSTAVO3025896;
GO


-- Habilita o contexto do banco de dados
USE LOJAGUSTAVO3025896;
GO
...
SELECT 
    ...
    t.create_date AS DataDeCriacao
FROM sys.tables t
...
SELECT 
    NumeroPedido  AS NumeroDoPedido, 
    CodigoProduto AS CodigoDoProduto, 
...
SELECT 
    CodigoVendedor AS CodigoVendedor, 
    NomeVendedor   AS NomeVendedor, 
    Salario        AS Salario
FROM VENDEDORES
WHERE NomeVendedor LIKE '[ACFM]%' 
...
SELECT 
    NumeroPedido AS NumeroDoPedido, 
    COUNT(DISTINCT CodigoProduto) AS ProdutosDiferentes, 
...
SELECT 
    ...
    p.DescricaoProduto AS NomeProduto, 
    p.ValorUnitario    AS PrecoUnitario, 
    ip.Quantidade      AS Quantidade, 
    (p.ValorUnitario * ip.Quantidade) AS ValorTotal, 
    o.PrazoEntrega     AS PrazoDeEntrega, 
    v.NomeVendedor     AS NomeVendedor
FROM ITEM_DE_PEDIDO ip
JOIN PEDIDOS o    ON ip.NumeroPedido = o.NumeroPedido
JOIN CLIENTES c   ON o.CodigoCliente = c.CodigoCliente
JOIN PRODUTOS p   ON ip.CodigoProduto = p.CodigoProduto
JOIN VENDEDORES v ON o.CodigoVendedor = v.CodigoVendedor
ORDER BY c.NomeCliente, p.DescricaoProduto, ip.Quantidade, o.PrazoEntrega, v.NomeVendedor;
GO
...
DROP DATABASE LOJAGUSTAVO3025896;
GO
